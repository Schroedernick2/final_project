...
class MainWindow(tk.Frame):
    ...
    def write_xml(self):
        if os.path.isfile(os.getcwd()+"\outTrackmodel.xml"):
            ...
            root = xml.etree.ElementTree.Element("Red Line")
            for outs in self.redtree.get_children():
                i = 1
                while i < 18:
                    xml.etree.ElementTree.SubElement(root, "red",
                                                 name=self.redtree.item(outs, "text")).text = \
                        self.redtree.item(outs, 'values')[i]
                    i += 1
